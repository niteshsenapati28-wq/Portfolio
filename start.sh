...
# Start the gateway IMMEDIATELY on Render's real public port, before
# anything else boots. This makes sure Render's automatic port detection
# locks onto the correct port right away, instead of briefly seeing the
# API's port first and getting confused (which caused repeated restarts).
echo "=== Starting gateway (frontend + proxy) on public port ${RENDER_PORT} ==="
cd /app
